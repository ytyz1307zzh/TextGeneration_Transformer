...
from transformer.Cos_Beam import Cos_Beam
import transformer.Constants as Constants

class Translator_cos(object):
    ...
    def translate_batch(self, src_batch):
        ...
        embed_mat=self.model.encoder.src_word_emb.weight.detach().numpy()
        ...
        wmd_weight=self.opt.wmd_weight

        #--- Prepare beams
        beams = [Cos_Beam(beam_size,lambda_1,lambda_2,lambda_3,wmd_weight,self.opt.cuda) for _ in range(batch_size)] #每个inst建一个beam类
        ...
        for i in range(self.model_opt.max_token_seq_len):
            ...
            for beam_idx in range(batch_size):
                ...
                if not beams[beam_idx].advance(word_lk.data[inst_idx],embed_mat,
                                               src_seq.view(n_remaining_sents, beam_size, -1).data[inst_idx]):
                    #下一个就是EOS，也算你翻译完了
